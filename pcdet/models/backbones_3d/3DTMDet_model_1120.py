...
class PatchDiffusion3D(nn.Module):
    ...
    def forward(self, x, coords_shift=1, diffusion_scale=3):
        ...
        for i in range(batch_size):
            ...
            selected_coords_expand[selected_coords_num * 1:selected_coords_num * 2, 3:4] = (
                        selected_coords_copy[:, 3:4] + diffusion_direction[:, 2:3] * coords_shift).clamp(min=0,
                                                                                                         max=w - 1)
            selected_coords_expand[selected_coords_num * 1:selected_coords_num * 2, 2:3] = (
            selected_coords_copy[:, 2:3]).clamp(min=0, max=h - 1)
            selected_coords_expand[selected_coords_num * 1:selected_coords_num * 2, 1:2] = (
            selected_coords_copy[:, 1:2]).clamp(min=0, max=d - 1)

            selected_coords_expand[selected_coords_num * 2:selected_coords_num * 3, 3:4] = (
            selected_coords_copy[:, 3:4]).clamp(min=0, max=w - 1)
            selected_coords_expand[selected_coords_num * 2:selected_coords_num * 3, 2:3] = (
                        selected_coords_copy[:, 2:3] + diffusion_direction[:, 1:2] * coords_shift).clamp(min=0,
                                                                                                         max=h - 1)
...
class GPTFBlock(nn.Module):
    def __init__(self, dim: int, depth: int, down_scales: list, window_shape, group_size_mamba, group_size_tf, direction, shift=False,
                 operator=None, layer_id=0, n_layer=0, window_nums_tf=[[1, 18, 18],[1, 6, 6]], if_mamba = True, if_Middle_layer = True, serializ='XYZ'):
        super().__init__()
...
class GPTF3DBackbone_vxmerging(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.linear_1 = GPTFBlock(self.layer_dim[0], depths[0], layer_down_scales[0], self.window_shape[0],
                                  ...
                                  window_nums_tf=self.window_nums_TF, if_mamba=self.if_mamba, if_Middle_layer=self.if_Middle_layer, serializ=self.serializ)  ##[27, 27, 32] --》 [13, 13, 32]
        ...
        self.linear_2 = GPTFBlock(self.layer_dim[1], depths[1], layer_down_scales[1], self.window_shape[1],
                                  ...
                                  window_nums_tf = self.window_nums_TF, if_mamba = self.if_mamba, if_Middle_layer = self.if_Middle_layer, serializ=self.serializ)
